...
func (m *Menu) Set(parent bool, key int, items menuItems) error {
	...
	if len(menus) > 0 {
		if m, ok := menus[fmt.Sprint(key)]; ok {
...
func (m *Menu) Get(ID int) (menuItems, error) {
	if items, ok := menus[fmt.Sprint(ID)]; ok {
		return items, nil
	}
...
func (m *Menu) String(key int) (string, error) {
	menuStr := ""
	items, err := m.Get(key)
	if err != nil {
		return menuStr, err
...
func GetMenuCollection() menuCollection {
	return menus
}
